...
public:
    Segment() = default;
    Segment(primitives::point_id_t a, primitives::point_id_t b, primitives::length_t length, const LengthMap& dt)
        : Connection(a, b), length(length)
        , xm(0.5 * (dt.x(a) + dt.x(b)))
        , ym(0.5 * (dt.y(a) + dt.y(b))) {}
    Segment(primitives::point_id_t a, primitives::point_id_t b, const LengthMap& dt)
        : Segment(a, b, dt.compute_length(a, b), dt) {}
    Segment(const Connection& c, const LengthMap& dt) : Segment(c.a, c.b, dt) {}
